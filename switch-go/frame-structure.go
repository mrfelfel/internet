...
// Ports number can be mutable due to physical links limits and endpoint must beware of this!
...
type frameStructure struct {
	NextHop        uint8  // Indicate switch port number use on next switch device! 0 use for multicast farmes to all ports!
	TotalHop       uint8  // Total hop numbers and also indicate payload location (Payload = rawFrame[frame.TotalHop+3:])!
	NextHeader     uint8  // Indicate upper layer protocol equal EtherType! 0 use for internal services in sRPC protocol!
	Switch0PortNum uint8  // Source Port Number. Not exist in multicast frames and will add by switch!
	Switch1PortNum uint8  // Any other than Switch0PortNum can be Destination Port Number. Not exist in multicast frames and will add by switch!
